...
padding:.1rem .55rem;font-size:.78rem;font-weight:600;cursor:pointer;
border:1px solid transparent}
.chip:hover{border-color:var(--acc);text-decoration:line-through}
.chip::after{content:" ×";opacity:.6;font-weight:700}
...
function renderSkills(){
  ...
  document.getElementById("picked").innerHTML =
    [...cur].map(id => `<span class="chip" data-rm="${id}" title="click to remove">${(SKILLS.find(s=>s.id===id)||{}).name||id}</span>`).join("")
    || `<span class="meta">nothing ticked yet — click a chip here to remove it</span>`;
}
...
document.getElementById("prev").onclick = () => { if(i>0) go(i-1); };
document.getElementById("next").onclick = () => { if(i<POSTINGS.length-1) go(i+1); };
